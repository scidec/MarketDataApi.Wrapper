...
{
	internal class StockCandles : Interfaces.BaseAPI, Interfaces.IStockCandles
	{
		private const string BaseUrl = @"https://api.marketdata.app/v1/stocks/candles/{resolution}/{stockSymbol}/?";

		public StockCandles(string apiToken, IWebProxy? proxy = null, string? source = null) : base(apiToken, proxy, source)
		{
		}

		public async Task<List<StockCandle>> V1StockCandlesAsync(Format? format, string? stockSymbol, string? resolution,
			...
			int? countBack = null, bool? extended = null, bool? adjustsplits = null)
		{
			...
			var result = await ExecuteQueryAsync<StockCandleResponse>(urlBuilder_.ToString());
			var total = result.H.Length;
			var items = new List<StockCandle>();

			for (int i = 0; i < total; i++)
			{

				items.Add(new StockCandle
				{
					Symbol = stockSymbol,
